...
```
┌─────────────────────────────────────────────────┐
│ 💧  Monstera Deliciosa (#0042)      [Done ✓]   │
│     Last watered 8 days ago                     │
├─────────────────────────────────────────────────┤
│ 💧  Pothos 'Golden' (#0003)         [Done ✓]   │
│     Overdue 2 days — needs water now!  🔴      │
└─────────────────────────────────────────────────┘
```
- Plant number displayed on care task row
...
**Edit Plant Modal** (opened from Edit button or plant number):
- Plant number input (editable, auto-filled on creation)
- Plant name input
- Location picker
- Save changes button
...
- Format: `#0042` (always 4-digit, zero-padded)
- On cards: small muted text, top-right
- On detail page: prominent in info row, clickable to edit
- On care task rows: next to plant name for quick reference
- Editable in plant profile / edit modal
...
|-------|-----------|---------|
| `individual_plants` | user_id, plant_number (4-digit auto-gen, unique per user), name, variety_id(opt), location, acquired_date, photo_filename | Owned houseplants with physical labeling number |
...
- `GET/POST /individual-plants` — houseplant CRUD (includes plant_number field)
- `PATCH /individual-plants/{id}/number` — update plant number
...
### Plant Number Generation Logic

- Auto-generate on plant creation: sequential 0001–9999 per user
- Wrap at 9999 → reuse deleted numbers
- Unique constraint: `(user_id, plant_number)`
- Editable anytime on plant detail page
...
- `types.ts` — extend with new types including plant_number
...
| **4** | My Plants | List, detail, add flow, care logging with plant numbers |
